...
from multiprocessing import Process, cpu_count, Event, current_process
from tasks.workermanager import queueManager
import gevent
...
@decorator
def concurrenttest(method, *args, **kwargs):
    #import gevent.monkey
    #gevent.monkey.patch_all()
    num_threads = 10
    workers =[]
    for i in range(num_threads):
        workers.append(Greenlet.spawn(method, args[0]))
    return workers

@decorator
def parallel(method, *args, **kwargs):
    def create_pool(generator, numworker):
        if isinstance(generator, BatchedDocumentGenerator):
            start, end = generator._doc_gen.getrange()
        else:
            start, end = generator.getrange()
        processtask = (end - start) / numworker
        for i in range(numworker):
            endpoint = start + processtask
            if i == numworker - 1:
                endpoint = end

            yield start, endpoint, i
            start += processtask
    ...
    generator = args[1]
    parallelmethod = args[3]
    for value in create_pool(generator, num_workers):
        start, endpoint, counter = value
        distributed_work = {'start': start, 'end': endpoint, 'counter': counter}
        '''
         The unlock_create or batch_create method takes generator as first argument
         using the decorator we replace that
         '''
        worker_process = Process(target=method, args=(args[0], distributed_work,(counter,num_workers), args[3]))
        '''
        Dont use deamon = True
...
class GenericLoadingTask(Thread, Task):
    ...
    def __init__(self, server, bucket, kv_store, batch_size=1, pause_secs=1, timeout_secs=60):
        ...
        self.batch_size = 200 #batch_size
    ...
    @concurrenttest
    def load_kvstore_parallel(self):
        while not self.signal.is_set():
            '''
            it will wait until last process set the signal.
            if signal not reached it will hang
            '''
            self.log.info(" waiting for signal to be set")
        qclient = queueManager().get_client(self.bucketname, self.beanstalkportnew)
        self.log.info(" Signal is set, started loading to KV Store using {}  from beanstalk {}".format(self.bucketname,qclient.watching()))
        while True:
            '''
            Condition until the Queue is Empty
            '''
            try:
                key, value = queueManager().consumer(qclient, self.bucketname)
                if value == "End":
                    break
                ...
                queueManager().close(qclient)
                raise e
        queueManager().close(qclient)

    @parallel
    def _unlocked_create(self, distributed_work, workerrange, parallelmethod = None):
        start, end = workerrange
        if start == end - 1:
            '''
             This is to make sure all threads will wait untill last
             process started
             '''
            self.signal.set()

        '''
            creating client instance for beanstalkd
        '''
        beanclient = queueManager().get_client(self.bucketname, self.beanstalkportnew)
        '''
        Being in Process copy of self.generator will be created
        '''
        ...
        while self.has_next():
            try:
                ...
                queueManager().producer(beanclient, key, value,self.priority)
                self.client.set(key, self.exp, self.flag, value)
            except Exception as e:
               queueManager().close(beanclient)
               raise e
        queueManager().close(beanclient)

    def _unlocked_read(self, partition, key, value=None):
        try:
            self.client.get(key)
    ...
    def _unlocked_read_replica(self, key, partition=None, value=None):
            self.client.getr(key)

    def _unlocked_update(self, key, partition, value=None):
        try:
    ...
    def _unlocked_delete(self, key, partition, value=None):
        try:
    ...
    def _unlocked_append(self, key, partition, value=None):
        try:
    ...
    @parallel
    def _batch_create(self, distributed_work, workerrange, parallel_method=None):
        start, end = workerrange
        localgenerator = self.generator
        ...
        while self.has_next():
            ...
            queueManager().producer(beanclient, None, key_val, self.priority)
        queueManager().close(beanclient)
        print "closed beanc"
    ...
    def _batch_delete(self, partition_keys_dic, key_val):
    ...
    def _read_batch(self, partition_keys_dic, key_val):
        self.client.getMulti(key_val.keys(), self.pause, self.timeout)
    ...
    def _populate_kvstore(self, partition_keys_dic, key_val):
        for part, keys in partition_keys_dic.items():
            tmp_part = dict(part)
            self._populate_kvstore_partition(tmp_part["partition"], keys, key_val)
...
class LoadDocumentsTask(GenericLoadingTask):
    bucketvalue = itertools.count()
    beanstalkport = itertools.count(14712)
    ...
    def __init__(self, server, bucket, generator, kv_store, op_type, exp, flag=0,
        ...
        self.priority = LoadDocumentsTask.bucketvalue.next()
        self.beanstalkportnew  = LoadDocumentsTask.beanstalkport.next()
        self.bucketname = 'bucket-' + str(self.priority)
        if proxy_client:
    ...
    def next(self):
        function = "_unlocked_" if self.batch_size == 1 else '_batch_'
        function += self.op_type
        try:
            if self.op_type == "create":
                #os.system('nohup beanstalkd -l 127.0.0.1 -p {} &'.format(self.beanstalkportnew))
                import subprocess
                import sys
                command = 'beanstalkd -l 127.0.0.1 -p {}'.format(self.beanstalkportnew)
                p = subprocess.Popen(command.split(), stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                getattr(self, function)(self.generator, (0,0), self.load_kvstore_parallel)
                print "process id", p.pid
                os.system("kill {}".format(p.pid))
                print "Done with Test"
                #self.load_kvstore_parallel()
            else:
                while self.has_next():
                    if self.batch_size == 1:
                        key, value = self.generator.next()
                        partition = self.kv_store.acquire_partition(key)
                        with Synchronized(partition) as tmp_part:
                            getattr(self, function)(key, tmp_part, value)
                    else:
                        key_value = self.generator.next_batch()
                        partition_keys_dic = self.kv_store.acquire_partitions(key_value.keys())
                        getattr(self, function)(partition_keys_dic, key_value)
        except (BaseException, MemcachedError) as error:
            self.set_exception(error)
...
class WorkloadTask(GenericLoadingTask):
    ...
    def _get_random_key(self):
        part, part_num = self.kv_store.acquire_random_partition()
        with Synchronized(part) as partition:
...
class ValidateDataTask(GenericLoadingTask):
    ...
    def _check_deleted_key(self, key):
        with Synchronized(self.kv_store.acquire_partition(key)) as partition:
            try:
                self.client.delete(key)
...
class VerifyMetaDataTask(GenericLoadingTask):
    def __init__(self, dest_server, bucket, kv_store, meta_data_store, max_err_count=100):
        ...
        self.num_valid_keys = len(self.valid_keys)
        self.num_deleted_keys = len(self.deleted_keys)
        self.keys_not_found = {self.client.rest.ip: []}
        self.itr = 0
        self.err_count = 0
        self.max_err_count = max_err_count
